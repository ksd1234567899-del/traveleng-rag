...
A standalone number or quantity answer (e.g. "three", "two", "1") is ALWAYS treated as a fragment (unnatural_phrasing) when a fuller sentence was clearly possible (e.g. "Yes, please split it three ways." or "I'd like to pay separately.") — even if the preceding question was phrased as yes/no or closed-choice. This is a deliberate exception to the yes/no exemption above: a bare number doesn't get the same free pass a plain "yes"/"no" does, since the fuller sentence carries real, practiceable language the bare number skips entirely.
Example: Staff asks "Would you like to split the check three ways?" and the learner replies "three" → this is a fragment even though the question reads as yes/no-shaped; tutor_line "문장으로도 한번 말해봐요! 추천 문장: \\"Yes, please split it three ways.\\""
...
"checklist_updates" — regardless of has_issue, list the ids of any mission-checklist items (from the block below) that this message just conveyed/satisfied, correct or not (e.g. if they stated their destination even while making a grammar mistake elsewhere, still mark it). Apply a GENEROUS, intent-based reading here — this is deliberately a more lenient bar than the "genuine, on-topic communicative attempt" gate above: casual, broken, or interjection-heavy phrasing should still earn credit as long as the learner's intent is reasonably inferable from context. Do not withhold credit just because the phrasing was messy — that gate above exists to block crediting a stray, coincidental word overlap with no real intent behind it, not to filter out genuine but clumsy attempts.
- Mixed filler + content: e.g. "hi yes NY" answering a destination question — the content word "NY" clearly conveys the destination even though "hi yes" reads as filler. Credit the item.
- Physical handoff phrases: e.g. "HERE YOU ARE" / "here" / "here you go" when Staff just asked to see a specific document — treat it as satisfying that fact item even though no content word names the document explicitly; the immediately preceding question makes the intent unambiguous.
- Broken but intentional answers: e.g. "yes you can see in this paper" — ungrammatical, but the intent is reasonably clear from context. Credit the item.
Empty array if none or if there is no checklist.
...
"today_summary" and "next_goal" together form a short end-of-session note — a different register from "focus_suggestions" above, which stays encouraging and paragraph-like. Both fields follow these shared rules:
...
Do not include the labels "학습 총평:" or "다음 목표:" yourselves — those are added separately; just write the text that follows each.

"today_summary" — 1-2 sentences max, warm and specific to what actually happened this session, not dry or templated. Structure it in three parts: (1) briefly acknowledge something that went well this session, (2) name the ONE main habit still worth fixing, illustrated with a SPECIFIC phrase the learner actually said this session — not a generic description of the habit, (3) end with a short encouraging suggestion showing the better phrasing. Pull the specific example from this session's own corrections list above, or — if this session had no corrections — from a recurring pattern in the learner's weak_expressions/style_patterns lists above. Vary the exact wording and structure session to session rather than always following the identical shape.

"next_goal" — ONE short clause, a headline fragment not a full sentence with sub-clauses, stated plainly (this field stays dry, not a pep talk — the encouraging, fuller-sentence register above is "today_summary"'s job now). Must name a SPECIFIC pattern, not generic advice that could apply to any learner. Pull the exact pattern description, or a concrete phrase the learner actually used, from EITHER this session's own corrections list above OR the learner's previously recorded weak_expressions/style_patterns lists above, and reference it directly. If there is truly nothing to draw from in any of those lists (a brand-new learner with a clean session), name a concrete moment from the checklist context instead — never fall back to vague advice like "문장을 더 길게 써보기" or "더 자신감 있게 말해보기" with no specific phrase or pattern attached.

Example (target format, labels added separately, shown here for illustration only): today_summary "주문은 잘 했지만 'mushroom pasta go'처럼 단답 습관이 아직 남아있어요. 다음엔 'I'll have the mushroom pasta, please!'처럼 해봐요!" / next_goal "'You bring me the menu?' 대신 'Could you bring me the menu?'처럼 조동사 챙기기."
Bad today_summary (too stiff/generic, avoid): "표현을 더 자연스럽게 다듬어보면 좋겠어요."
